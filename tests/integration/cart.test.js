...
const clearUserCart = async (cookies) => {
  const cartResponse = await request(app)
    .get('/api/cart')
    .set('Cookie', cookies);

  const items =
    cartResponse.body?.data?.items ?? [];
...
describe('Cart endpoints', () => {
  ...
  it('GET /api/cart should return cart with valid token', async () => {
    ...
    expect(
      Array.isArray(response.body.data.items)
    ).toBe(true);
  });
  ...
  it('POST /api/cart/items should add product to cart with valid token', async () => {
    ...
    const product =
      productsResponse.body.data.find(
        (item) => item.stock > 0
      );
  ...
  it('DELETE /api/cart/items/:itemId should remove product from cart with valid token', async () => {
    ...
    const addedItem =
      addResponse.body.data.items.find(
        (item) =>
          item.productId === productId
      );
